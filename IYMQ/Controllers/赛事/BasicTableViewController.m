...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.view setFrame:(CGRect){0,70,MainWidth,MainHeight - 120}];
    ...
    // Map item to a cell
    //
    //    self.manager[@"ListImageItem"] = @"ListImageCell"; // which is the same as
    [self.manager registerClass:@"ListImageItem" forCellWithReuseIdentifier:@"ListImageCell"];
...
- (void)addItems
{
    ...
    for (int i = 0; i < 12; i++) {
        ...
        SHlistModel *list = [SHlistModel new];
        list.imgString = @"图片默认底图.png";
        list.picString = @"比赛中.png";
        list.nameString = @"第一届“i羽毛球杯”混合团体";
        list.BSstring = @"比赛：2015.11.15-11.20";
        list.BMstring = @"报名：2015.11.05-11.10";
...
//    for (SHlistModel *dictionary in items) {
...
//        [self.manager addSection:section];
        ...
        // Add item (image)
        //
        [section addItem:[ListImageItem itemWithImageNamed:list]];
